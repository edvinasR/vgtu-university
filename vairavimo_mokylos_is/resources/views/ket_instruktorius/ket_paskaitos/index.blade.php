        ...
        <div class="row wide" >
            ...
            <div class="col-md-9" id="cal_container">
                <div class="panel panel-default" style="min-height: -webkit-fill-available;">
                    <div class="panel-heading">Paskaitų kalendorius</div>
	    ...
	    <div class="modal-dialog">
	      ...
	      <div class="modal-content">
	        <div class="modal-header">
	          ...
	          <h4 class="modal-title">Sukurti/atnaujniti paskaitą</h4>
	        </div>
	        <div class="modal-body">
	          
	                    <form method="POST"  id="paskaitosIvedimoForma" action="/ket_instruktorius/paskaitos" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
                            {{ csrf_field() }}
                            @include ('ket_instruktorius.ket_paskaitos.form')
     ...
     <div class="modal fade" id="paskaitosRedagavimas" role="dialog">
	    <div class="modal-dialog">
	      ...
	      <div class="modal-content">
	        <div class="modal-header">
	          <button type="button" class="close" data-dismiss="modal">&times;</button>
	          <h4 class="modal-title">Atnaujinti paskaitą</h4>
	        </div>
	        <div class="modal-body">
  
		                    <form id="paskaitosAtnaujinimoForma" method="POST" action="/ket_instruktorius/paskaitos" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
	                            {{ method_field('PATCH') }}
	                            {{ csrf_field() }}
	                            @include ('ket_instruktorius.ket_paskaitos.editForm')
     ...
     <div class="modal fade" id="eksportas" role="dialog">
	    <div class="modal-dialog" style="margin-right: 100px;">
	      ...
	      <div class="modal-content">
	        ...
	        <div class="modal-footer">
	         	<a id="eksportoNuoroda" data-nuoroda="{{ url('/export/tvarkarastis/'.Auth::user()->getRoleEntityId()) }}" href="" title="Eksportuoti"><button class="btn btn-primary btn-xs"> Eksportuoti</button></a>
	        </div>
	      </div>
  ...
  <script>
	function populateModal(event,type){
		$('.errorMsg').addClass('hide');
			if(type =="PUT"){
				$("#paskaitosAtnaujinimoForma").attr('action', "/ket_instruktorius/paskaitos/"+event.id);	
				...
				$("#grupiu_pasirinkmimasEdit").val(event.grupes);
...
// DuomenÅ³ Å¡altinis kalendoriui
   ...
   $(document).ready(function() {
	   ...
	   $("#naikiniti").click(function(e) {
		   if(confirm("Ar tikrai norite iÅ¡trinti Å¡iÄ… pasakitÄ…?"))
		       $.ajax({
		              type: "DELETE",
		              url: $("#paskaitosAtnaujinimoForma").attr('action'),
		         	  beforeSend: function (xhr) {
