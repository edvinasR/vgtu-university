                    ...
                    <form class="form-horizontal" method="POST" action="{{ route('register') }}">
                        ...
                        <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                          ...
                          <div class="form-group{{ $errors->has('surename') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <input id="surename" type="text" class="form-control" name="surename" value="{{ old('surename') }}" required autofocus>
                        ...
                        <div class="form-group{{ $errors->has('teises_FK') ? ' has-error' : '' }}">
                            <label for="teises_FK" class="col-md-4 control-label">Teisės</label>

                            <div class="col-md-6">
                                <select id="teises_FK" class="form-control"  name="teises_FK" value="{{ old('teises_FK') }}" required autofocus>
                                	@foreach($teises as $teise)
                                	<option value={{$teise -> id}}>{{$teise -> pavadinimas}}</option>>
                                	@endforeach
                                
                                </select>

                                @if ($errors->has('teises_FK'))
                                    <span class="help-block">
                                        <strong>{{ $errors->first('teises_FK') }}</strong>
                                    </span>
                                @endif
                            </div>
                        </div>
