<div class="form-group {{ $errors->has('tema') ? 'has-error' : ''}}">
    ...
    <div class="col-md-6">
        <select class="form-control" name="instruktorius" type="number" id="mokinys" value="{{ $zinute->instruktorius or ''}}" required>
        @foreach( $mokiniai as $key => $mokinys)
    		<option value={{$key}} {{isset($zinute->instruktorius) && $key ==  $zinute->instruktorius?  'selected="selected"': ''}}>{{$mokinys}}</option>
	    @endforeach
	    </select>
